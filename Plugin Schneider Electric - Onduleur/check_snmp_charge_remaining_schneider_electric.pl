...
##
## OID permettant de récupérer le pourcentage de charge de la batterie
## upsAdvBatteryCapacity dans la MIB Specifique powernet416.mib
##
my $snmp_oid_charge_remaining = ".1.3.6.1.4.1.318.1.1.1.2.2.1.0";


##
## Permet de faire un get sur une feuille (OID)
##
my $resultOID = $session->get_request(-varbindlist => [$snmp_oid_charge_remaining]);
...
##
my $result = $resultOID->{$snmp_oid_charge_remaining};
...
##
$output = "Battery Capacity: $result% (Seuils W:$opt_w% C:$opt_c%)";
$perfdata = "Charge-Remaining=$result;$opt_w;$opt_c;0;100";	 

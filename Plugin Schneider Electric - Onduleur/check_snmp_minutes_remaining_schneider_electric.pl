...
##
## OID permettant de récupérer le temps de fonctionnement sur batterie de l'onduleur
## upsAdvBatteryRunTimeRemaining
##
my $snmp_oid_minutes_remaining = ".1.3.6.1.4.1.318.1.1.1.2.2.3.0";


##
## Permet de faire un get sur une feuille (OID)
##
my $resultOID = $session->get_request(-varbindlist => [$snmp_oid_minutes_remaining]);
...
##
my $result = $resultOID->{$snmp_oid_minutes_remaining};

##
## Utilisation de la fonction split pour créer un tableau avec la chaine de caractères 
##
@tab = split (" ", $result);
$result = $tab[0];
...
##
$output = "Minutes Remaining: $result min (Seuils W:$opt_w C:$opt_c)";
$perfdata = "Minutes-Remaining=$result;$opt_w;$opt_c;0;60";	 
